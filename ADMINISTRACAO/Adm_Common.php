...
function CarregaTabelaConcursos() {
	...
	foreach($IteradorConcursos as $CodConcurso => $Concurso) {
		...
		$Str .= '    <td class="Centro">'.$Concurso->retornaString(STR_CONCURSOELEITORAL).': ['.$Campos['CodConcurso'].'] '.$Concurso->get("descricao").' (de '.$Concurso->get("datahorainicio", datahora).' até '.$Concurso->get("datahorafim", datahora).') ';
		...
		if($Direito && ($Concurso->get("situacaoconcurso") < SITUACAOCONCURSO_HOMOLOGADO)) {
            $Str .= '<a href="ELC_Valida_Pessoas_Concurso.php?CodConcurso='.$Campos['CodConcurso'].'">[Homologação]</a>
			       <a href="ELC_Valida_Pessoas_Concurso.php?CodConcurso='.$Campos['CodConcurso'].'&Final">[Homologação final]</a> ';
		...
		foreach($Eleicoes as $CodEleicao => $Eleicao) {
		  ...
		  if($Ativo && $Zerada && !$EmAberto) {
			  ...
			  $ConsultaVotantes = new Consulta($SQL);
				...
				$Str .= "Ativo. ".$Porcentagem."% dos eleitores já votaram.";
			}
			else {
                if($Encerrou) {
                    if($JaContou) {
                        $Str .= 'Encerrada. <a href="ELC_Apuracao.php?CodConcurso='.$CodConcurso.'&CodEleicao='.$CodEleicao.'">Visualizar a apuração</a>';
                    }
					else
