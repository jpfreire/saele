...
elseif($Concurso->estadoConcurso() == CONCURSO_ENCERRADO) {
    ...
    <table width="85%" border="0" cellspacing="0" cellpadding="0" class="tabela" align="center">
        <tr>
            <td>
            Contagem dos votos realizada em <?=$Contagem->get("dataoperacao", data)?>, às <?=$Contagem->get("dataoperacao", hora)?>.
            </td>
        </tr>
        <?php
        ...
        <tr>
            <td>
            Recontagem dos votos realizada em <?=$Recontagem->get("dataoperacao", data)?>, às <?=$Recontagem->get("dataoperacao", hora)?>.
            </td>
        </tr>
        <?php
        } ?>
...
<a href="ELC_Relatorio_Votos.php?CodConcurso=<?=$Eleicao->get("codconcurso")?>&amp;CodEleicao=<?=$Eleicao->get("codeleicao")?>&ord=nr">Relatório de Votos (por número de <?=$Concurso->retornaString(STR_CHAPA)?>)</a><br />
<a href="ELC_Relatorio_Votos.php?CodConcurso=<?=$Eleicao->get("codconcurso")?>&amp;CodEleicao=<?=$Eleicao->get("codeleicao")?>&ord=votos">Relatório de Votos (por ordem de colocação)</a>
