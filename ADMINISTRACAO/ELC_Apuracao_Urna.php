...
if(isset($_GET['CodUrna']) && (trim($_GET['CodUrna']) != "")) {
  ...
  <a href="javascript: void(0);" onclick="javascript: janela = window.open('ELC_Relatorio_Votos.php?CodConcurso=<?=$Concurso->getChave()?>&CodEleicao=<?=$Eleicao->getChave()?>&CodUrna=<?=$Urna->getChave()?>&ord=nr', 'hhg', 'top=5, left=10');">Relatório de Votos (por número de chapa)</a><br />
  <a href="javascript: void(0);" onclick="javascript: janela = window.open('ELC_Relatorio_Votos.php?CodConcurso=<?=$Concurso->getChave()?>&CodEleicao=<?=$Eleicao->getChave()?>&CodUrna=<?=$Urna->getChave()?>&ord=votos', 'hhg', 'top=5, left=10');">Relatório de Votos (por ordem de colocação)</a>
