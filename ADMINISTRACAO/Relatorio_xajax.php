...
<table width="85%" border="1" cellspacing="0" cellpadding="0" class="tabela" align="center">
    <tr bgcolor="#d3d3d3">
        ...
        <td align="center" class="a2">
            ...
            <a href="javascript: ListaEleitoresImpressao(false);">[Imprimir para divulgação]</a>
...
<table>
    ...
    <tr class="Linha1">
        ...
        if(count($Participacoes) == 0)
          $HTML .= '
                <option value=""> -- Nenhuma participação cadastrada. Preencha o campo abaixo -- </option>';
        else {
    ...
    <tr class="Linha2">
        <td style="text-align: center; font-size: 12px;">
          Ou preencha um novo tipo de participação:<br />
            ...
            switch($Form['Acao']) {
                case "C": //Candidato
                    ...
                    elseif(trim($Form['TipoParticipacao']) != "")
                      ...
                      return $objResponse->alert("Selecione a participação, ou informe um novo tipo de participação.");
...
function CarregaEdicaoParticipacao($CodChapa, $CodPessoaEleicao) {
    ...
    $Chapa = $Eleicao->devolveChapa($CodChapa);
    if($Chapa === null)
      return $objResponse->alert("Chapa inválida!");
    
    $Candidato = $Chapa->devolveCandidato($CodPessoaEleicao);
    if($Candidato === null)
      return $objResponse->alert("Candidato inválido!");
...
function SalvaParticipacao($Form) {
    ...
    if(trim($Form['NovoTipoParticipacao']) == "" && trim($Form['TipoParticipacao']) == "")
      return $objResponse->alert("Selecione a participação, ou informa um novo tipo de participação.");
    ...
    elseif(trim($Form['TipoParticipacao']) != "") {
      ...
      if($Participacao->novo())
        return $objResponse->alert("Participação inválida!");
    }
